...
public class AudioUtils {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                if (!info.getName().startsWith("OMX.")) {
                    ...
                    // using MediaCodec.
                    Log.i(TAG, "skipping '" + info.getName() + "'.");
                    continue;
                }
    ...
    public static void showMetrics(MediaFormat format, int numBytesSubmitted, int numBytesDequeued) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            Log.i(TAG, "queued a total of " + numBytesSubmitted + " bytes, " + "dequeued " + numBytesDequeued + " bytes.");
            ...
            Log.i(TAG, "desiredRatio = " + desiredRatio + ", actualRatio = " + actualRatio);
        }
    }
    ...
    public static void showSomeBytes(String tag, byte[] bytes) {
        Log.i(TAG, "enc: " + tag + ": length: " + bytes.length);
        String str = "";
        ...
        if (len > 0) {
            ...
            Log.i(TAG, "enc: " + tag + ": hex: " + str);
        }
    }

    public static boolean getMicroPhoneIsMute(Context context) {
