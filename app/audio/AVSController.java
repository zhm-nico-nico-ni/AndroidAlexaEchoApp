...
import com.amazon.alexa.avs.AlertEventListener;
import com.amazon.alexa.avs.AlertHandler;
import com.amazon.alexa.avs.AlertManager;
import com.amazon.alexa.avs.AlertManagerFactory;
import com.amazon.alexa.avs.AlertScheduler;
import com.amazon.alexa.avs.AlertsFileDataStore;
import com.amazon.alexa.avs.AudioInputFormat;
import com.amazon.alexa.avs.DialogRequestIdAuthority;
import com.amazon.alexa.avs.DirectiveDispatcher;
...
import com.amazon.alexa.avs.AudioInputFormat.Constants;
...
public class AVSController implements RecordingStateListener, AlertHandler, AlertEventListener,
        DirectiveDispatcher, AVSAudioPlayer.AlexaSpeechListener, ParsingFailedHandler, WakeWordDetectedHandler {
    ...
    public SpeechRequestAudioPlayerPauseController speechRequestAudioPlayerPauseController; //TODO maybe need
...
//    private WakeWordIPC wakeWordIPC = null;
    ...
    public AVSController(AlertManagerFactory alarmFactory, AVSClientFactory avsClientFactory,
...
//        }
        ...
        this.player = new AVSAudioPlayer(this);
...
//                Arrays.asList(listenHandler, speechRequestAudioPlayerPauseController));
        expectSpeechListeners = new HashSet<>();
        ...
        // synchronize state
        alertManager.loadFromDisk(new AlertManager.ResultListener() {
            @Override
...
//                .setChannelIndexMask (Constants.channels) // 需要转换
            .build();

        controller.microphone = AudioCapture.getAudioHardware(MyApplication.getContext(), format);
    }
...
//        }
    ...
    private InputStream getMicrophoneInputStream(AVSController controller,
        ...
        throw new RuntimeException("microphone not available ");// LineUnavailableException();
    }
...
//                    scheduler.cancel();
//                }
                Log.w(Log.TAG_APP, "alertManager.hasAlert ");
            }
...
//            alertManager.add(alert);
            ...
            Log.w(Log.TAG_APP, "alertManager.DeleteAlert : " + payload.getToken());
        } else {
            throw new DirectiveHandlingException(ExceptionType.UNSUPPORTED_OPERATION,
