...
import com.amazon.alexa.avs.FilesUtil;
...
public class AVSAudioPlayer {
    ...
    private MyMediaPlayer audioPlayer;
...
//    private final AudioPlayerProgressReporter progressReporter;
    ...
    public AVSAudioPlayer(AVSController controller) {
        ...
        currentlyMuted = audioManager.isStreamMute(STREAM_TYPE);
    ...
    public void handlePlay(Play play) throws DirectiveHandlingException {
        ...
        if (stream.hasAttachedContent()) {
            try {
                ...
                FilesUtil.copyReplaceExisting(stream.getAttachedContent(), tmp.getPath());
...
//        audioPlayer.getMediaPlayer().setVolume(currentVolume);
        float volume = currentVolume / VLCJ_MAX_VOLUME;
    ...
    public void handleSetMute(SetMute setMutePayload) {
        ...
        float volume = currentlyMuted ? 0 : currentVolume / VLCJ_MAX_VOLUME;
        audioPlayer.setVolume(volume, volume);
        controller
...
//                audioPlayerStateMachine.playbackFailed();
        ...
        audioPlayer.setOnCompletionListener(new MyMediaPlayer.IOnCompleteListener() {
            @Override
...
//                || audioPlayerStateMachine.getState() == AudioPlayerState.BUFFER_UNDERRUN);
    ...
    private boolean playItem(Stream stream) {
        synchronized (audioPlayer) {
            ...
            log.debug("playing {}", url+" offset:"+offset);
            ...
            try {
                ...
                if (stream.hasAttachedContent()) { //TODO
                    Log.w(Log.TAG_APP, "stream.hasAttachedContent() ");
...
//        return new PlaybackStatePayload(latestStreamToken, offset, playerState.toString());
        return new PlaybackStatePayload(latestStreamToken, 0, ""); //TODO
    }
...
//        }
        offsetInMilliseconds = audioPlayer.getCurrentPosition();
...
//                    int bufferSizeInBytes, int mode
                    ...
                    try {
                        ...
                        speaker.play(); // 启动音频设备，下面就可以真正开始音频数据的播放了
                        // 打开mp3文件，读取数据，解码等操作省略 ...
                        byte[] buffer = new byte[4096];
                        int count = inpStream.read(buffer);
                        while(count > 0)
                        {
